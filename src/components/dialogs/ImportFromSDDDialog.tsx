...
interface ImportFromSDDDialogProps {
  open: boolean;
...
export function ImportFromSDDDialog({ open, onOpenChange, data, onSubmit }: ImportFromSDDDialogProps) {
        ...
        if (prev >= 90) {
          ...
          setExtractedRequirements([
            ...
            },
            {
              ...
              description: 'System shall perform automated daily backups with point-in-time recovery capability.',
              confidence: 0.85,
              selected: false
            }
          ]);
  ...
  const handleRequirementEdit = (id: string, field: 'title' | 'description', value: string) => {
    setExtractedRequirements(prev =>
      prev.map(req => req.id === id ? { ...req, [field]: value } : req)
    );
  };

  const handleProceedToFolder = () => {
    setStep('folder');
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Import Requirements from SDD</DialogTitle>
        </DialogHeader>
        ...
        {step === 'review' && (
          <div className="space-y-6">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleProceedToFolder} disabled={selectedCount === 0}>
                Proceed to Folder Selection ({selectedCount})
              </Button>
            </div>
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleCreateRequirements}>
                Proceed to Create New Requirements
              </Button>
            </div>
          </div>
        )}
